...
class NotesRepositoryImpl : NotesRepository {
    ...
    override fun addNote(note: Note) = dataSource.insertNote(me.meenagopal24.notesxpert.db.Notes(id = note.id, title = note.title, body = note.body, createdAt = note.createdAt.toLong()))

    override fun deleteNote(id: Long) = dataSource.deleteNote(id)

    override fun getNoteById(id: Long) = dataSource.getNoteById(id)?.let {
        Note(id = it.id, title = it.title, body = it.body, createdAt = it.createdAt.toInt())
    }
